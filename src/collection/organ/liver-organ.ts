...
class SugarStatefulIconSubState extends NumberStatefulIconSubState {
  changeValueBy(gameState: GameState, changeBy: number): void {
    if (changeBy > 0) {
      ...
      const stomach = PlayableUtils.findConcrete<StomachOrgan>(
        'stomach',
        gameState.targetState
      );
...
export class LiverOrgan extends OrganPlayable {
  ...
  getDescription(): Description[] {
    return [
      ...
      { code: 'Posiada status {{health_up}}' },
      {
        code:
          'Reguluje poziom {{cubes}} jego brak obniża {{health_normal}} wszystkich organów'
      }
    ];
  }

  getSubStates() {
    return [...super.getSubStates(), this.sugar];
  }
  ...
  constructor(state?: FirebaseStatefulIcon) {
    super('liver', state);
    this.sugar = new SugarStatefulIconSubState(
      'cubes',
      'sugar',
      state ? state.values['sugar'] || 10 : 10
    );
